...
import { updateTeam, addTeam, removeTeam } from './teamSlice';
...
export const getTeams = createAsyncThunk(
  ...
  async (params, { dispatch, getState }) => {
    try {
      ...
      dispatch(
        showMessage({ message: 'Get Team List error', variant: 'error' })
      );
      throw error;
    }
  }
...
export const { selectAll: selectTeams, selectById: selectTeamsById } =
  teamsAdapter.getSelectors((state) => state.teamsApp.teams);
...
export const selectGroupedFilteredTeams = createSelector(
  [selectFilteredTeams],
  (teams) => {
    return teams
      .sort((a, b) =>
...
export const { setTeamsSearchText } = teamsSlice.actions;
