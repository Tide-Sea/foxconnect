...
import { acceptOrganization } from '../store/organizationSlice';
...
const OrganizationItem = (props) => {
  ...
  function onSelect() {
    if (organization) {
      console.log('[organization] ', organization.status);
      if (organization.status === 'padding') {
        console.log('[organization] ', organization.organizationId);
  ...
  return (
    <Card className="flex flex-col relative items-start w-full h-full p-24 rounded-lg shadow border-inherit hover:shadow-xl transition-shadow duration-150 ease-in-out overflow-visible">
      ...
      <div className="flex flex-col flex-auto justify-end w-full">
        {/* {activation && activation.package && activation.package.name && (
          <div className="flex items-center mt-12 text-md font-md">
            <Typography color="text.secondary">Package:</Typography>
            <Typography className="mx-4 truncate">{activation.package.name}</Typography>
          </div>
        )}
        {activation && activation.description && (
          <div className="flex items-center mt-12 text-md font-md">
            <Typography color="text.secondary">Description:</Typography>
            <Typography className="mx-4 truncate">{activation.description}</Typography>
          </div>
        )}
