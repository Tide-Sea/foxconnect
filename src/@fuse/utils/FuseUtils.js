...
class FuseUtils {
  ...
  static formatMentionToText(textMessage) {
    if (!textMessage) {
      return '';
    }
    const patternMention = /@\[(.*?)\)/gi;
    const mentionList = textMessage.match(patternMention);
    if (mentionList) {
      ...
      for (
        ...
        const pattern = /\((.*?)\)|\[(.*?)\]+/gi;
        const mention = mentionList[mentionIndex].match(pattern);
        const name = mention[0].replace(/[[\]']+/g, '');
        const mentionId = mention[1].replace(/[()]+/g, '');
        // textMessage = textMessage.replace(mentionList[mentionIndex], `**@${name}**`);
        textMessage = textMessage.replace(mentionList[mentionIndex], `<b>@${name}</b>`);
      }
    }
  ...
  static formatTextToMention(textMessage, entityMap) {
    if (entityMap) {
  ...
  static removeDuplicateMembers(array) {
    const members = array.filter(
      (member, index) =>
        index === array.findIndex((other) => member.createdBy.id === other.createdBy.id)
    );
    return members;
  }
}
